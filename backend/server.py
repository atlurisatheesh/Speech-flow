...
# ── Emergent Universal LLM Key (OpenAI-compatible proxy) ──────
# Powers Whisper STT, GPT chat/agent logic, and TTS without a user OpenAI key.
EMERGENT_LLM_KEY = os.environ.get("EMERGENT_LLM_KEY")
EMERGENT_BASE_URL = os.environ.get("INTEGRATION_PROXY_URL", "https://integrations.emergentagent.com") + "/llm"

def make_llm_client() -> "AsyncOpenAI":
    """Return an AsyncOpenAI client routed through the Emergent universal-key proxy."""
    return AsyncOpenAI(api_key=EMERGENT_LLM_KEY, base_url=EMERGENT_BASE_URL)
...
async def generate_chat_completion(system_message: str, prompt: str, temperature: float = 0.3, model: str = "gpt-4o-mini") -> str:
